...
```bash
# Setup (one-time)
cd ~/.codex/skills/hack/scripts && pip install -r requirements.txt && playwright install chromium
...
|---|---|
| "扫一下这个站" / scan site | `hack_scan.py <URL>` |
| "看看信息" / recon only | `hack_scan.py <URL> --recon-only` |
| "开个 CDP" / Chrome 调试 | `cdp_launch.py` (stealth ON) |
| "模拟真人" / human behavior | `cdp_launch.py --human` |
| "抓包" / capture traffic | `cdp_launch.py --proxy localhost:8080` |
| "AI 模糊测试" / adaptive fuzz | `ai_fuzzer.py <URL> --param <P>` or `--goal sqli` |
| "认证审计" / audit auth | `auth_auditor.py <URL>` |
| "全面攻击" / full pentest | `orchestrator.py <URL>` or Workflow P (below) |
| "子域名枚举" / subdomain enum | Module 1 |
| "端口扫描" / port scan | Module 2 |
| "测 SSRF" | Module 3 |
| "测 CORS" | Module 4 |
| "测 XXE" | Module 5 |
| "测竞态" / race condition | Module 6 |
| "测文件上传" / file upload | Module 7 |
| "分析 JS" / JS analysis | Module 8 |
| "测请求走私" / smuggling | Module 9 |
| "测 Host 头" / host header | Module 10 |
| "TLS 审计" / SSL test | Module 11 |
| "Nuclei 扫描" | Module 12 |
| "隐藏参数" / Mass Assignment | Module 15 |
| "找源站" / CDN bypass | Module 16 |
| "拖 git" / git dump | Module 17 |
| "盲打 XSS" / Blind XSS | Module 18 |
| "GraphQL 深度测试" | Module 19 |
| "业务逻辑" / business logic | Module 20 |
| SQLi / XSS / SSTI / cmdi / path_traversal | `ai_fuzzer.py <URL> --goal <GOAL>` |
| CSRF | Check anti-CSRF tokens on state-changing requests |
| Open Redirect | `vuln_scan.py` — `check_open_redirect()` |
| IDOR | Change numeric IDs (`?id=123` → `?id=124`) |
| XML-RPC (WP) | POST to `/xmlrpc.php` with `system.listMethods` |
| User Enumeration (WP) | GET `/wp-json/wp/v2/users/` |
| OAuth/JWT/SAML | `auth_auditor.py` |
| "完整侦察" / full recon | Module 1 → Module 2 → hack_scan.py → Module 8 |
| "完整攻击链" / full kill chain | See Workflow P below |
...
> **All module code below assumes the CDP connection above is already established. Each snippet shows only the unique logic.**
